...
% Initialize priority queue (node, g-cost, h-cost, f-cost), visited array, parent array, and costs
queue = {startNode, 0, 0, 0, 'None'};  % Priority queue: node, g-cost, h-cost, f-cost, parent
...
while true
    ...
    for k = 1:size(queue, 1)
        ...
        frontierStr = frontierStr + queue{k, 1} + "(" + num2str(queue{k, 2}) + "," + num2str(queue{k, 4}) + "," + queue{k, 5} + ")";
    end
    ...
    elseif isempty(queue)
        ...
        currentHCost = queue{minFCostidx, 3}; % Get the corresponding h-cost
        currentFCost = queue{minFCostidx, 4}; % Get the corresponding f-cost
        currentParent = queue{minFCostidx, 5}; % Get the parent of the current node
    ...
    for i = 1:size(neighbors, 1)
        ...
        for j = 1:length(nodes)
            if strcmp(nodes{j}, neighbor)
                ...
                newFCost = newHCost;  % f = h "Best-First*" search
                if newGCost < costs(j)  % If the new cost is cheaper
                    ...
                    if ~ismember(neighbor, queue(:, 1))
                        queue(end + 1, :) = {neighbor, newGCost, newHCost, newFCost, current};  % Enqueue the neighbor with the new g, h and f costs and parent
                    else
                        % Update the g, h and f cost in the queue if this path is cheaper
                        for k = 1:size(queue, 1)
                            if strcmp(queue{k, 1}, neighbor) && queue{k, 2} > newGCost 
                                queue{k, 2} = newGCost;
                                queue{k, 3} = newHCost;
                                queue{k, 4} = newFCost;
                                queue{k, 5} = current; % Update the parent in the queue
                                break;
                            end
                        end
                    end
