...
method='AStar'; % Available methods: BFS, DFS, Uniform, BestFirst, AStar

% Call the function
[path, cost, stepTable] = search_with_table(graph, startNode, goalNodes, method);
...
function [path, cost, stepTable] = search_with_table(graph, startNode, goalNodes, method)
...
while true
    ...
    if strcmp(method, 'DFS') % loop direction based on the method
        loopRange = size(neighbors, 1):-1:1; % Reverse for DFS
    else
        loopRange = 1:size(neighbors, 1); % for all the other methods
    end
            ...
            if strcmp(nodes{j}, neighbor)
                ...
                newHCost = hcosts_table{neighbor, 1};
                    ...
                    elseif strcmp(method, 'Uniform')
                        newFCost = newGCost;
                    elseif strcmp(method, 'BestFirst')
                        newFCost = newHCost;
                    else
                        newFCost = 0;
                    end
